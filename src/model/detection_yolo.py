...
counting_regions = [

    {
        ...
        "polygon": Polygon([(0, 0), (0, 320), (320, 320), (320, 0)]),
        "counts": 0,
        "dragging": False,
...
def run(
    ...
    # Video setup
    videocapture = cv2.VideoCapture(int(source))

    fps = FPS().start()

    frame_width, frame_height = int(
        videocapture.get(3)), int(videocapture.get(4))
    write_fps, fourcc = int(videocapture.get(
        5)), cv2.VideoWriter_fourcc(*"mp4v")

    # Output setup
    save_dir = increment_path(
        Path("./src/logs/ultralytics_rc_output") / "exp", exist_ok)
    save_dir.mkdir(parents=True, exist_ok=True)
    video_writer = cv2.VideoWriter(str(
        save_dir / f"{Path(source).stem}.mp4"), fourcc, write_fps, (frame_width, frame_height))
    ...
    while videocapture.isOpened():
        ...
        if view_img:
            ...
            cv2.putText(frame, text, (15, int(origin_h * 0.92)),
...
def parse_opt():
    ...
    parser.add_argument("--weights", type=str,
                        default="src/model/checkpoints/yolov10n.pt", help="initial weights path")
    parser.add_argument("--device", default="0",
                        help="cuda device, i.e. 0 or 0,1,2,3 or cpu")
    parser.add_argument("--source", type=str,
                        required=True, help="video file path")
    ...
    parser.add_argument("--line-thickness", type=int,
                        default=2, help="bounding box thickness")
    parser.add_argument("--track-thickness", type=int,
                        default=2, help="Tracking line thickness")
    parser.add_argument("--region-thickness", type=int,
                        default=4, help="Region thickness")
