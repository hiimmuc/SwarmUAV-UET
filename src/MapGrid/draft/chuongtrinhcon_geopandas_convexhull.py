...
def main():
    ...
    def add_polygon():
        global polygon_1
        new_map = []
        hull = ConvexHull(position_list)
        new_map = [position_list[vertex] for vertex in hull.vertices]
        print(f"POLYGON: ")
        polygon_1 = map_widget.set_polygon(new_map)
        for index, position in enumerate(new_map):
            print(
                f"Point {index + 1} set at latitude: {position[0]}, longitude: {position[1]}")

    def add_path():
        global path_1
        path_1 = map_widget.set_path(position_list)

    def clear_path():
        global path_1
        path_1 = path_1.delete()
    ...
    def calculate_area_and_display(label):
        ...
        hull = ConvexHull(position_list)
        new_map = [position_list[vertex] for vertex in hull.vertices]
        area_m2 = calculate_geographic_area(new_map)
    ...
    btn_clear = tk.Button(root, text="Xóa Đa Giác", command=clear_map_widget)
