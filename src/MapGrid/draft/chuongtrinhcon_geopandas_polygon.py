...
from tinhtoanS import shoelace_area, draw_polygon, calculate_area_and_display, is_point_inside_polygon
from inoutfile import read_points_from_file, write_points_to_file
import tkinter
...
def main():
    ...
    map_widget = tkintermapview.TkinterMapView(root, width=1200, height= 550, corner_radius=0)
    ...
    def left_click_event(coordinates_tuple):
        ...
        marker_list.append(map_widget.set_marker(latitude, longitude, text= "Diem " + str(number))) #tao marker
        print_point_list.append(GeoPosition(latitude, longitude)) #them diem vao danh sach
    ...
    def open_file():
        ...
        if filename:
            ...
            for x, y in zip(x_coords, y_coords):
                number += 1
                marker_list.append(map_widget.set_marker(x, y, text= "Diem " + str(number))) #tao marker
                print_point_list.append(GeoPosition(x, y)) #them diem vao danh sach
                ...
                for index, position in enumerate(print_point_list):
                    print(f"Point {index + 1} set at latitude: {position.latitude}, longitude: {position.longitude}")
    ...
    def calculate_area_and_display(label):
        ...
        if area_m2 is not None:
            label.config(text=f"Diện tích của đa giác là: {area_m2:.2f} square meters")
        else:
            ...
            for x in range(-400, 400, int(distance)):
                for y in range(-300, 300, int(distance)):
                    if is_point_inside_polygon(x, y, x_coords, y_coords):
                        map_widget.create_oval(x + 398, 302 - y, x + 402, 298 - y, fill="blue")
            grid_enabled = True
    ...
    btn_clear_last_point = tk.Button(frame_input, text="Xóa Điểm Trước Đó", command=clear_last_point)
    ...
    btn_open_file = tk.Button(frame_input, text="Xuất File", command = export_file)
    ...
    btn_create_polygon = tk.Button(frame_input, text="Tạo đa giác", command=add_polygon)
    btn_create_polygon.grid(row=2, column=5, padx=10)

    btn_create_path = tk.Button(frame_input, text="Tạo đường đi", command=add_path)
    btn_create_path.grid(row=2, column=7, padx=10)

    btn_create_path = tk.Button(frame_input, text="Xóa đường đi", command=clear_path)
    btn_create_path.grid(row=2, column=8, padx=10)
    ...
    btn_calculate.config(command=lambda: calculate_area_and_display(label_area))
    ...
    btn_toggle_grid = tk.Button(frame_grid, text="Bật/Tắt Lưới", command=toggle_grid)
    btn_toggle_grid.grid(row=0, column=2, padx=10)

    btn_export_grid_points = tk.Button(frame_grid, text="Xuất Điểm Lưới", command=export_grid_points)
