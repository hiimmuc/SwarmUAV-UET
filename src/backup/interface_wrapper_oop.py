...
from PyQt5.QtWidgets import QMainWindow, QMessageBox, QTableWidgetItem, QDialog, QDialogButtonBox, QLabel, QVBoxLayout
...
stackedWidget_indexes = {'main page': 0,
                         'map page': 1}

tabWidget_indexes = {'all': 0,
                     'uav1': 1,
                     'uav2': 2,
                     'uav3': 3,
                     'uav4': 4,
...
class App(QMainWindow):
    ...
    def init_application(self):
        ...
        # logo
        self.ui.page_name.setPixmap(QtGui.QPixmap(
            f"{basePath}/assets/icons/logo1.png"))
        self.ui.page_name.setScaledContents(True)
        self.ui.page_name_2.setPixmap(QtGui.QPixmap(
            f"{basePath}/assets/icons/logo1.png"))
        self.ui.page_name_2.setScaledContents(True)
        self.ui.logo2_2.setPixmap(QtGui.QPixmap(
            f"{basePath}/assets/icons/logoUET.png"))
        ...
        for screen in self.uav_general_screen_views:
            screen.setPixmap(QtGui.QPixmap(
                f"{basePath}/assets/pictures/nosignal.jpg"))
            screen.setScaledContents(True)
        for screen in self.uav_stream_screen_views:
            screen.setPixmap(QtGui.QPixmap(
                f"{basePath}/assets/pictures/no_signal2.jpg"))
            screen.setScaledContents(True)
    ...
    # //-/////////////////////////////////////////////////////////////
    # //-/////////////////////////////////////////////////////////////
    def template_information(self, uav_index,
    ...
    def switch_with_menuBar(self, stack_name, tab_name):
        self.ui.stackedWidget.setCurrentIndex(
            stackedWidget_indexes[stack_name])
    ...
    def map_button_event(self):
        self.ui.btn_arm.clicked.connect(
            lambda: asyncio.create_task(
                self.uav_fn_arm(self.active_tab_index)))

        self.ui.btn_disarm.clicked.connect(
            lambda: asyncio.create_task(
                self.uav_fn_disarm(self.active_tab_index)))

        self.ui.btn_open_close.clicked.connect(
            lambda: asyncio.create_task(
                self.uav_fn_openClose(self.active_tab_index)))

        self.ui.btn_landing.clicked.connect(
            lambda: asyncio.create_task(
                self.uav_fn_landing(self.active_tab_index)))

        self.ui.btn_takeOff.clicked.connect(
            lambda: asyncio.create_task(
                self.uav_fn_takeoff(self.active_tab_index)))

        self.ui.btn_pause.clicked.connect(
            lambda: asyncio.create_task(
                self.uav_fn_pause(self.active_tab_index)))

        self.ui.btn_connect.clicked.connect(
            lambda: asyncio.create_task(
                self.uav_fn_connect(self.active_tab_index)))

        self.ui.btn_return.clicked.connect(
            lambda: asyncio.create_task(
                self.uav_fn_return(self.active_tab_index)))

        self.ui.btn_mission.clicked.connect(
            lambda: asyncio.create_task(
                self.uav_fn_mission(self.active_tab_index)))

        self.ui.btn_pushMission.clicked.connect(
            lambda: asyncio.create_task(
                self.uav_fn_pushMission(self.active_tab_index)))
    ...
    async def uav_fn_disarm(self, uav_index):
        if uav_index != 0:
            ...
            info = self.template_information(
                uav_index, arming_status='DISARMED')
            await asyncio.sleep(1)
    ...
    async def uav_fn_connect(self, uav_index):
        if uav_index != 0:
            ...
            info = self.template_information(
                uav_index, connection_status='Connected')
    ...
    async def uav_fn_pushMission(self, uav_index):
        if uav_index != 0:
            ...
            info = self.template_information(
                uav_index, mode_status='Push Mission')
            await asyncio.sleep(1)
    ...
    async def uav_fn_openClose(self, uav_index):
        if uav_index != 0:
            ...
            info = self.template_information(
                uav_index, mode_status='Open' if UAVs[uav_index]['uav'].openClose_status else 'Close')
            await asyncio.sleep(1)
