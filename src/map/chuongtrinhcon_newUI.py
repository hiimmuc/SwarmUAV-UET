...
def main():
    ...
    Drone_image = ImageTk.PhotoImage(Image.open(os.path.join(
        parent_dir, "images", "Drone.png")).resize((20, 20)))
    dot_image = ImageTk.PhotoImage(Image.open(os.path.join(
        parent_dir, "images", "Dot.png")).resize((10, 10)))
    ...
    def open_file_drone():
        ...
        for i in range(drone_num):
            try:
                with open(f'{parent_dir}/logs/gps/gps_data{x_coords[i]}.txt', 'r') as file:
    ...
    rotated_drone_images = create_rotated_images(
        f"{parent_dir}/images/Drone.png", rotations, size=(50, 50))
