...
class MAVSDKServer:
    ...
    def __init__(
        ...
        if MAVSDK_SERVER_PATH.exists():
            ...
            self.command = (
                f"python3 src/mavsdk_server_shell.py {connection_url} " f"-p {port} -i {id}"
            )

        # Informational message
        self.init_msg = f"[INFO] Starting MAVSDK server {id} with {connection_url} -p {port}"
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline"]):
        try:
            ...
            if proc.info["name"] == "mavsdk_server" or (
                proc.info["cmdline"]
                and any("mavsdk_server" in arg for arg in proc.info["cmdline"])
            ):
...
if __name__ == "__main__":
    ...
    test_server = MAVSDKServer(
        id=1, protocol="udp", server_host="127.0.0.1", port=50051, bind_port=14540
    )
