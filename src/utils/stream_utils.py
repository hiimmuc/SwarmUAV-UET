        ...
        if self.is_writer_opened():
            ...
            if (frame.shape[1], frame.shape[0]) != self.writer_frameSize:
                resized_frame = cv2.resize(
                    frame, dsize=self.writer_frameSize, interpolation=cv2.INTER_AREA
                )
    ...
    def run(self) -> None:
        ...
        while self.isRunning:
            ...
            if not self.stream.connect():
                ...
                print(
                    f">>> Lost streaming signal, trying to reconnect in {reconnect_delay/1000:.1f}s "
            ...
            while self.stream.is_capture_opened() and self.isRunning:
                ...
                if self.model is not None:
                    try:
                        ...
                        inference_time = (
                            speeds["preprocess"] + speeds["inference"] + speeds["postprocess"]
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source", type=str, default="0", help="Video source (file, URL, or device index)"
    )
    parser.add_argument("--record", action="store_true", help="Record the stream to a video file")
    parser.add_argument("--output", type=str, default="output.mp4", help="Output video filename")
    args = parser.parse_args()

    # Configure stream
    capture_config = {"address": args.source, "index": 0}
