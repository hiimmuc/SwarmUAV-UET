...
# Initialize logger
logger = get_logger(
    name="serial_utils", console_level="info", file_level="debug", log_file="rtk_serial.log"
)
...
def find_base_station_port(
    baud_rate: int = DEFAULT_BAUD_RATE, timeout: float = DEFAULT_TIMEOUT, verbose: bool = False
) -> Optional[str]:
    """
    ...
    for port in ports:
        try:
            ...
            with serial.Serial(port=port.device, baudrate=baud_rate, timeout=timeout) as ser:
...
async def send_rtcm(
    ...
    try:
        ...
        with serial.Serial(
            port=base_station_port, baudrate=baud_rate, timeout=DEFAULT_TIMEOUT
        ) as ser:
        ...
        logger.info(
            f"RTCM forwarding stopped. Sent {packets_sent} packets, {bytes_sent} bytes total."
        )
